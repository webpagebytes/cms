...
public class FlatStorageImporterExporterEx {
	private static final Logger log = Logger.getLogger(FlatStorageImporterExporterEx.class.getName());
	...
	Map<String, WPBUri> urisStorage = new HashMap<String, WPBUri>();
	Map<String, WPBPage> pagesStorage = new HashMap<String, WPBPage>();
	Map<String, WPBPageModule> modulesStorage = new HashMap<String, WPBPageModule>();
	Map<String, WPBArticle> articlesStorage = new HashMap<String, WPBArticle>();
	Map<String, WPBMessage> messagesStorage = new HashMap<String, WPBMessage>();
	Map<String, WPBParameter> parametersStorage = new HashMap<String, WPBParameter>();
	Map<String, WPBFile> filesStorage = new HashMap<String, WPBFile>();
	WPBProject projectStorage;
   ...
   public void importFromZipStep2(InputStream is) throws WPBIOException
    ...
    public void importFromZipStep3() throws WPBIOException
    {
    	...
    	try                                                        
    	{
			...
			if (tempProject == null)
			{
			    projectStorage.setExternalKey(WPBProject.PROJECT_KEY);
				dataStorage.addWithKey(projectStorage);
			} else
			{
				dataStorage.update(projectStorage);				
			}
	
	    	for(String key: urisStorage.keySet())
	    	{
	    		dataStorage.addWithKey(urisStorage.get(key));
	    	}
	    	for(String key: pagesStorage.keySet())
	    	{
	    		dataStorage.addWithKey(pagesStorage.get(key));
	    	}
	    	for(String key: modulesStorage.keySet())
	    	{
	    		dataStorage.addWithKey(modulesStorage.get(key));
	    	}
	    	for(String key: articlesStorage.keySet())
	    	{
	    		dataStorage.addWithKey(articlesStorage.get(key));
	    	}
	    	for(String key: messagesStorage.keySet())
	    	{
	    		dataStorage.addWithKey(messagesStorage.get(key));
	    	}
	    	for(String key: parametersStorage.keySet())
	    	{
	    		dataStorage.addWithKey(parametersStorage.get(key));
	    	}
	    	for(String key: filesStorage.keySet())
	    	{
	    		dataStorage.addWithKey(filesStorage.get(key));
	    	}
    	} catch (Exception e)
    	{
            log.log(Level.SEVERE, e.getMessage(), e);
            throw new WPBIOException("cannot import step 3", e);   		
    	}
    	finally
    	{
    		dataStorage.startNotifications();
	...
	public void importUri(ZipInputStream zis) throws WPBIOException
	{
		try 
		{
			...
			if (uri != null)
			{
				...
				urisStorage.put(uri.getExternalKey(), uri);
			}
		} catch (IOException e)
		{
			log.log(Level.SEVERE, e.getMessage(), e);			
	...
	public void importWebPage(ZipInputStream zis) throws WPBIOException
	{
		try 
		{
			...
			if (webPage != null)
			{
				pagesStorage.put(webPage.getExternalKey(), webPage);
			}
		} catch (IOException e)
		{
			log.log(Level.SEVERE, e.getMessage(), e);			
	...
	public void importWebPageSource(ZipInputStream zis, String path) throws WPBIOException
	{
		try 
		{
			...
			if (pagesStorage.containsKey(externalKey))
			{
				byte[] content = getBytesFromInputStream(zis);
				String pageContent = new String(content, "utf-8");

				pagesStorage.get(externalKey).setHtmlSource(pageContent);
				pagesStorage.get(externalKey).setHash(WPBPage.crc32(pageContent));	
			} else
			{
	...
	public void importWebPageModuleSource(ZipInputStream zis, String path) throws WPBIOException
	{
		try 
		{
			...
			if (modulesStorage.containsKey(externalKey))
			{
				byte[] content = getBytesFromInputStream(zis);
				String pageContent = new String(content, "utf-8");

				modulesStorage.get(externalKey).setHtmlSource(pageContent);
			} else
			{
	...
	public void importFileContent(ZipInputStream zis, String path) throws WPBException
	{
		try
		{
			...
			WPBFile file = filesStorage.get(externalKey);
			String filePath = "";
			String dirPath = FileController.getDirectoryFullPath(file.getOwnerExtKey(), filesStorage);
			if (dirPath.length()>0)
			{
			...
			filesStorage.put(externalKey, file);
		} catch (IOException e)
		{
			log.log(Level.SEVERE, e.getMessage(), e);						
		}
	...
	public void importArticleSource(ZipInputStream zis, String path) throws WPBIOException
	{
		try
		{
			...
			if (articlesStorage.containsKey(externalKey))
			{
				byte[] contentBinary = getBytesFromInputStream(zis);
				String stringContent = new String(contentBinary, "utf-8");
				articlesStorage.get(externalKey).setHtmlSource(stringContent);
			} else
			{
	...
	public void importProject(ZipInputStream zis) throws WPBIOException
	{
		try 
		{
			...
			projectStorage = importer.buildProject(props);;
		} catch (IOException e)
		{
			log.log(Level.SEVERE, e.getMessage(), e);			
		}
	...
	public void importPageModule(ZipInputStream zis) throws WPBIOException
	{
		try 
		{
			...
			{
				modulesStorage.put(webPageModule.getExternalKey(), webPageModule);
			}
		} catch (IOException e)
		{
			log.log(Level.SEVERE, e.getMessage(), e);			
	...
	public void importArticle(ZipInputStream zis) throws WPBIOException
	{
		try 
		{
			...
			if (article != null)
			{
				articlesStorage.put(article.getExternalKey(), article);
			}
		} catch (IOException e)
		{
			log.log(Level.SEVERE, e.getMessage(), e);			
	...
	public void importFile(ZipInputStream zis) throws WPBIOException
	{
		try 
		{
			...
			if (file != null)
			{
				filesStorage.put(file.getExternalKey(), file);
			}
		} catch (IOException e)
		{
			log.log(Level.SEVERE, e.getMessage(), e);			
	...
	public void importMessage(ZipInputStream zis) throws WPBIOException
	{
		try 
		{
			...
			if (message != null)
			{
				messagesStorage.put(message.getExternalKey(), message);
			}
		} catch (IOException e)
		{
			log.log(Level.SEVERE, e.getMessage(), e);			
	...
	public void importParameter(ZipInputStream zis) throws WPBIOException
	{
		try 
		{
			...
			if (parameter != null)
			{
				parametersStorage.put(parameter.getExternalKey(), parameter);
			}
		} catch (Exception e)
		{
			log.log(Level.SEVERE, e.getMessage(), e);
